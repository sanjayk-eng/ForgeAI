...
import { updateProject, updateRepositoryBranch } from "../../../api/projects.api";
  ...
  return (
    <div className="fixed inset-0 z-30 grid place-items-center bg-[var(--overlay)] p-5 backdrop-blur-md">
      <form className="relative w-full max-w-[520px] rounded-xl border border-forge-accent/20 bg-forge-card p-7 shadow-2xl" onSubmit={submit}>
        ...
        <div className="mt-6 grid gap-4"><label className="block"><span className="label">Project name</span><input className="input mt-2" value={name} onChange={(event) => setName(event.target.value)} autoFocus /></label><label className="block"><span className="label">Description</span><textarea className="input mt-2 min-h-24 resize-y" value={description} onChange={(event) => setDescription(event.target.value)} /></label>{project.repository && <label className="block"><span className="label">GitHub branch</span><input className="input mt-2" value={branch} onChange={(event) => setBranch(event.target.value)} placeholder="main" /></label>}</div>
