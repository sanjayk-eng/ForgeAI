...
function RepositoryFooter({ repository, syncing, onSync }: { repository: NonNullable<Project["repository"]>; syncing: boolean; onSync: () => void }) {
  return <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-white/[0.08] pt-4 text-xs text-forge-soft"><div className="flex flex-wrap items-center gap-x-5 gap-y-2"><span className="inline-flex items-center gap-2"><FolderGit2 size={14} className="text-forge-accent" />{repository.github_owner}/{repository.github_repository_name}</span><span className="font-mono text-forge-muted">{repository.default_branch}</span></div><button className="inline-flex items-center gap-2 border border-white/[0.12] px-3 py-2 text-[10px] font-extrabold uppercase tracking-[.08em] text-forge-soft transition hover:border-forge-accent/40 hover:text-forge-text disabled:cursor-not-allowed disabled:opacity-50" onClick={onSync} disabled={syncing} title="Sync repository"><RefreshCw size={13} className={syncing ? "animate-spin" : ""} />{syncing ? "Syncing" : repository.sync_status === "FAILED" ? "Retry sync" : "Sync now"}</button></div>;
}
